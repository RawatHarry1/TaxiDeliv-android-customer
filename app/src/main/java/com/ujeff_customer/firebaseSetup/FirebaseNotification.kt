package com.ujeff_customer.firebaseSetup
...
import com.ujeff_customer.view.activity.walk_though.ui.home.HomeFragment
...
class FirebaseNotification : FirebaseMessagingService() {
    ...
    /**
     * Pending Intent Handle For Main
     * */
    private fun getPendingIntentForMain(destinationId: Int, bundle: Bundle? = null): PendingIntent =
        if (Build.VERSION.SDK_INT >= 31) NavDeepLinkBuilder(this).setComponentName(MainHome::class.java)
            .setGraph(R.navigation.main_mobile_navigation).setDestination(destinationId)
            .setArguments(bundle).createTaskStackBuilder().getPendingIntent(
                0, PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT
            )!!
        else NavDeepLinkBuilder(this).setComponentName(MainHome::class.java)
            .setGraph(R.navigation.main_mobile_navigation).setDestination(destinationId)
            .setArguments(bundle).createPendingIntent()
