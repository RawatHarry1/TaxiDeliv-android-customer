...
class MyEarningsFragment : BaseFragment<FragmentMyEarningsBinding>() {
    ...
    private fun setClicks() {
        ...
        binding.tvFilter.setOnClickListener {
            requireActivity().arrayAdapter(
                autoCompleteTextView = binding.tvFilter,
                list = listOf("All", "Today", "This Month")
            ) {
                filter = it
                viewModel.getEarningData(filter)
            }
        }
    }

    private fun setBookingsUI() {
        binding.tvFilter.isVisible = false
    ...
    private fun observeBookingList() =
        viewModel.bookingHistoryData.observeData(viewLifecycleOwner, onLoading = {
            showProgressDialog()
        }, onSuccess = {
            hideProgressDialog()
    ...
    private fun observeEarningData() = viewModel.earningData.observeData(viewLifecycleOwner,
        onLoading = {
            ...
            hideProgressDialog()
            binding.tvBalance.text = "${SharedPreferencesManager.getCurrencySymbol()} ${
                this?.totalEarnings.orEmpty().ifEmpty { "0.0" }.formatAmount()
            }"
