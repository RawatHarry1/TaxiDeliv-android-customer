...
class RideRepo @Inject constructor(
    ...
    suspend fun uploadPackageImage(
        part: MultipartBody.Part, hashMap: HashMap<String, RequestBody?>
    ) = flow {
    ...
    suspend fun updatePackageStatus(
        ...
        emit(apiInterface.updatePackageStatus(requestBody = JSONObject().apply {
            ...
            if (cancellationReason != null) put("cancelltion_reason", cancellationReason)
            if (!packageImages.isNullOrEmpty()) put("package_images", JSONArray(packageImages))
            if (isEnd) {
                put("is_for_end", "1")
