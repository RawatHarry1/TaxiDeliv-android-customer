...
import com.superapp_customer.databinding.FragmentTripsBinding
        ...
        when (textView.id) {
            R.id.tvTrip -> {
                if (SharedPreferencesManager.getInt(SharedPreferencesManager.Keys.SELECTED_OPERATOR_ID) == 1) {
                    ...
                    binding.tvTrip.text = getString(R.string.title_trips)
                } else {
                    binding.tvTitle.text = getString(R.string.delivery_history)
                    binding.tvTrip.text = getString(R.string.title_delivery)
                }
