...
class SearchLocationVM @Inject constructor(
    ...
    val searchLocationData: LiveData<ApiState<List<CreateRideData.LocationData>>> = _searchLocationData
    ...
    fun searchPlaces(searchText: String) = viewModelScope.launch {
        rideRepo.searchPlaces(search = searchText).onStart {
            ...
            _searchLocationData.value = ApiState.error(ErrorModel(message = error.localizedMessage.orEmpty()))
        }.collectLatest {
            if (it.isSuccessful){
                    ...
                    if (has("results")){
                        getJSONArray("results").let { predictions ->
                            for (index in 0 until predictions.length()){
                                ...
                                val latitude = prediction.optJSONObject("geometry")?.optJSONObject("location")?.optString("lat").orEmpty()
                                val longitude = prediction.optJSONObject("geometry")?.optJSONObject("location")?.optString("lng").orEmpty()
                                val placeId = prediction.optString("place_id").orEmpty()
                                CreateRideData.LocationData(
                                    latitude = latitude, longitude = longitude, address = address, placeId = placeId
                                ).apply {
                                    array.add(this)
                                }
                            }
                ...
                _searchLocationData.value = ApiState.error(ErrorModel(message = it.errorBody()?.string().toString()))
            }
        }
    }
